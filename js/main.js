			...
			$.getJSON('https://nodes-testnet.wavesnodes.com/addresses/data/3N9eE86dXUm7rfc2WWCMLHkaEM4Y8yoNj7u/MaxGovernTokenDepositerKey')
					...
					if (FundBoxHeight[0].value == 0 ) {
						...
						if (r < 0) {
							...
							if (HighestPushAddress[0].value == Address+"_Push") {
								document.getElementById("Rewarding").innerHTML = leftPad(Hours, 2)+' <h1>Hours'+ leftPad(Minutes, 2)+' minutes before reward is released</h1> <br> Your Push is actually the highest, if reward is released you would be the winner'
							}else{
								document.getElementById("Rewarding").innerHTML = leftPad(Hours, 2)+' <h1>Hours'+ leftPad(Minutes, 2)+'minutes before reward is released</h1> minutes before reward is released '	
							}
						}
